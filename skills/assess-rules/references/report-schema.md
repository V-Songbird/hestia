...
The full schema covers the markdown report template, --fix mode diff format, and JSON output schema with all field definitions.

Path conventions: `.hestia-tmp/` for temp files, `.hestia-ignore` for excluded-rules.

Grade intervals: A ≥ 0.80, B ≥ 0.65, C ≥ 0.50, D ≥ 0.35, F < 0.35. Precision: 3 decimal JSON / 2 decimal markdown. Schema version: "0.1".
